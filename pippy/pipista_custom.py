...
Taken from github.com/pudquick/pipista.py on 4/2/16
Slightly modified.
...
def _chunk_read(response, chunk_size=32768, report_hook=None, filename=None):
    ...
    if (filename == None) and (response.info().has_key('Content-Disposition')):
        ...
        try:
            filename = \
            response.info()['Content-Disposition'].split('filename=')[1]
...
def _download(src_dict, print_progress=True):
    ...
    if print_progress:
        data, filename = _chunk_read(response, report_hook=_chunk_report,
                                     filename=output)
    else:
        data, filename = _chunk_read(response, report_hook=None,
                                     filename=output)
    if (len(data) > 0):
        try:
            f = open(filename, 'wb')
...
def install_xmlrpclib(path='.'):
    ...
    r = requests.get(
        'http://hg.python.org/cpython/raw-file/70274d53c1dd/Lib/xmlrpclib.py')
...
if _auto_path:
    ...
    lib_dir = os.path.join(mod_dir, 'tmp_pippy')
